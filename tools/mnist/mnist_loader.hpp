...
class MNISTLoader {
  ...
  public:
    ...
    auto load_imgages(std::string const &path, int batch_size) {
        ...
        std::vector<tensor::Tensor> images(nb_batches);
        std::vector<ftype> batch_host(batch_size * rows * cols);

        for (size_t b = 0; b < nb_batches; ++b) {
            auto batch_tensor = tensor::tensor(batch_size, 1, (int)rows, (int)cols);
    ...
    auto create_output_tensor(int *label, int batch_size) {
        auto batch_gpu = tensor::tensor(batch_size, 1, 10, 1);
