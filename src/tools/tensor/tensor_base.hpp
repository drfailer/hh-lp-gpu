...
class TensorBase {
  public:
    ...
    TensorBase(TensorShape const &shape)
        : shape_(shape),
          size_(shape.size()),
          desc_(descriptor_from_shape(shape)) { }
    ...
    void data(T *data) { this->data_ = data; }
    ...
    auto from_host(T *host) {
        return memcpy_host_to_gpu(this->data_, host, this->size_);
    }

    // assums that the host array has the proper size
    auto to_host(T *host) const {
        return memcpy_gpu_to_host(host, this->data_, this->size_);
    }
